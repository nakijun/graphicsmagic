...
type
  ...
  public
    ...
    procedure DrawCropBorder(const ACanvas: TCanvas; const AOffsetVector: TPoint);
    procedure DrawCropHandles(const ACanvas: TCanvas; const AOffsetVector: TPoint);
...
procedure TgmCrop.DrawCropBorder(const ACanvas: TCanvas;
...
procedure TgmCrop.DrawCropHandles(const ACanvas: TCanvas;
  const AOffsetVector: TPoint);
var
...
begin
  ...
  if SquareContainsPoint(  Point( FCropStart.X, (FCropEnd.Y - FCropStart.Y) div 2 + FCropStart.Y ),
                           HANDLE_RADIUS, Point(AX, AY)  ) then
  begin
    Result := dhLeftHalfAYBY;
  end
  else
  if SquareContainsPoint(  Point( FCropEnd.X, (FCropEnd.Y - FCropStart.Y) div 2 + FCropStart.Y ),
                           HANDLE_RADIUS, Point(AX, AY)  ) then
  begin
    Result := dhRightHalfAYBY;
  end
  else
  if SquareContainsPoint(  Point( (FCropEnd.X - FCropStart.X) div 2 + FCropStart.X, FCropStart.Y ),
                           HANDLE_RADIUS, Point(AX, AY)  ) then
  begin
    Result := dhTopHalfAXBX;
  end
  else
  if SquareContainsPoint(  Point( (FCropEnd.X - FCropStart.X) div 2 + FCropStart.X, FCropEnd.Y ),
                           HANDLE_RADIUS, Point(AX, AY) ) then
  begin
