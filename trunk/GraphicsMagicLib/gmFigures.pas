...
//-- TgmFigureObject -----------------------------------------------------------
  ...
  public
    ...
    procedure DrawFigure(ACanvas: TCanvas; const PenMode: TPenMode;
      ...
      const FigureDrawMode: TgmFigureDrawMode); virtual; abstract;
...
//-- TgmFigureList -------------------------------------------------------------
  ...
  public
    ...
    function IfPointOnFigure(const X, Y:Integer): Boolean;
...
function TgmFigureObject.GetHandleAtPoint(
...
begin
  ...
  case FFlag of
    ffStraightLine:
      begin
        if SquareContainsPoint( FStartPoint, ARadius, Point(AX, AY) ) then
        begin
          Result := dhLineStart;
        end
        else
        if SquareContainsPoint( FEndPoint, ARadius, Point(AX, AY) ) then
        begin
          Result := dhLineEnd;
        end;
      end;
        ...
        begin
          ...
          begin
            case FCurveControl of
              ccpFirst:
                begin
                  if SquareContainsPoint( FCurvePoint1, ARadius, Point(AX, AY) ) then
                  begin
              ...
              ccpSecond:
                begin
                  if SquareContainsPoint( FCurvePoint2, ARadius, Point(AX, AY) ) then
                  begin
    ...
    ffPolygon, ffRegularPolygon:
      begin
        ...
        for i := Low(FPolygonPoints) to High(FPolygonPoints) do
        begin
          if SquareContainsPoint( FPolygonPoints[i], ARadius, Point(AX, AY) ) then
          begin
    ...
    ffRectangle, ffRoundRectangle, ffEllipse:
      begin
        ...
        if SquareContainsPoint(Point(FStartPoint.X, FEndPoint.Y), ARadius, Point(AX, AY)) then
        begin
          Result := dhAxBy;
        end
        else
        if SquareContainsPoint(Point(FEndPoint.X, FStartPoint.Y), ARadius, Point(AX, AY)) then
        begin
          Result := dhBxAy;
        end
        else
        if SquareContainsPoint( Point(FStartPoint.X, (FEndPoint.Y - FStartPoint.Y) div 2 + FStartPoint.Y), ARadius, Point(AX, AY) ) then
        begin
          Result := dhLeftHalfAYBY;
        end
        else
        if SquareContainsPoint( Point(FEndPoint.X, (FEndPoint.Y - FStartPoint.Y) div 2 + FStartPoint.Y), ARadius, Point(AX, AY) ) then
        begin
          Result := dhRightHalfAYBY;
        end
        else
        if SquareContainsPoint( Point((FEndPoint.X - FStartPoint.X) div 2 + FStartPoint.X, FStartPoint.Y), ARadius, Point(AX, AY) ) then
        begin
          Result := dhTopHalfAXBX;
        end
        else
        if SquareContainsPoint( Point((FEndPoint.X - FStartPoint.X) div 2 + FStartPoint.X, FEndPoint.Y), ARadius, Point(AX, AY) ) then
        begin
...
function TgmFigureObject.ContainsPoint(const TestPoint: TPoint): Boolean;
var
  i: Integer;
begin
  Result := False;
  ...
  begin
    Result := NearLine(TestPoint, FStartPoint, FEndPoint);
  end
  else
  if FFlag = ffCurve then
  begin
    Result := PointOnCurve(FStartPoint.X, FStartPoint.Y, FCurvePoint1.X, FCurvePoint1.Y,
                           FCurvePoint2.X, FCurvePoint2.Y, FEndPoint.X, FEndPoint.Y,
                           TestPoint);
  end
  else
  if FFlag in [ffPolygon, ffRegularPolygon] then
  begin
    ...
    begin
      if NearLine(TestPoint, FPolygonPoints[i], FPolygonPoints[i + 1]) then
      begin
        Result := True;
        Break;
      end;
  ...
  if (FFlag = ffRectangle) or
     ...
     (FFlag = ffCircle) then
  begin
    Result := Windows.PtInRect( Rect(FStartPoint.X, FStartPoint.Y, FEndPoint.X, FEndPoint.Y), TestPoint );
  end;
end;
...
function TgmFigureList.SelectedContainsPoint(
  const TargetPoint: TPoint): Boolean;
var
  LFound: Boolean;
  i       : Integer;
begin
  ...
  begin
    if TgmFigureObject(Self.Items[i]).IsSelected then
    begin
      LFound := TgmFigureObject(Self.Items[i]).ContainsPoint(TargetPoint);
    end;
...
function TgmFigureList.IfPointOnFigure(const X, Y:Integer): Boolean;
var
  i         : Integer;
  LFigureObj: TgmFigureObject;
begin
  ...
  begin
    ...
    begin
      ...
      if LFigureObj.ContainsPoint( Point(X, Y) ) then
      begin
        Result := True;
        Break;
      end;
