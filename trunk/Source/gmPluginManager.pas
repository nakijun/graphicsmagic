  ...
  
  {
    TODO :
      * Split this in two different parts
        ...
        - And then the PluInGUI lib, because IMHO
...
type
  ...
  public
    ...
    procedure UpdatePluginMenuEnableState(const AChannelType: TgmWorkingChannelType);
  ...
  public
    ...
    procedure UpdatePluginMenusEnableState(const AChannelType: TgmWorkingChannelType);
  end;
...
procedure TgmPluginInfo.UpdatePluginMenuEnableState(
  const AChannelType: TgmWorkingChannelType);
begin
  if Assigned(FPluginMenu) then
  begin
    if AChannelType = wctRGB then
    begin
