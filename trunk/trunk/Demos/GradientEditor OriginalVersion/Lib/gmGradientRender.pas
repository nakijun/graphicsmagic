...
{ Graphics32 }
  GR32,
{ GraphicsMagic Lib }
  gmGradient, GR32_Add_BlendModes, gmTypes;
...
type
  ...
  private
    FColorGradient       : TgmColorGradient;
  ...
  public
    ...
    property ColorGradient         : TgmColorGradient      read FColorGradient        write SetColorGradient;
  ...
  procedure DrawLinearGradient(const ABitmap: TBitmap32;
    const AStartPoint, AEndPoint: TPoint; const AColorGradient: TgmColorGradient;
  ...
  procedure DrawDiamondGradient(const ABitmap: TBitmap32;
    const AStartPoint, AEndPoint: TPoint; AColorGradient: TgmColorGradient;
    const AReverse: Boolean);
...
procedure TgmGradientRender.SetColorGradient(const AValue: TgmColorGradient);
begin
...
procedure HorizLinearGradientFill(const ABitmap: TBitmap32;
  const AX1, AX2: Integer; const AColorGradient: TgmColorGradient;
  const AReverse: Boolean);
var
  i                 : Integer;
...
end; { HorizLinearGradientFill }

// vertical linear gradient
procedure VertLinearGradientFill(const ABitmap: TBitmap32;
  const AY1, AY2: Integer; const AColorGradient: TgmColorGradient;
  const AReverse: Boolean);
var
  i                 : Integer;
  LWidth, LHeight   : Integer;
...
begin
  ...
  // do nothing if the start point equals to end point
  if (AStartPoint.X = AEndPoint.X) or
     (AStartPoint.Y = AEndPoint.Y) then
  begin
    Exit;
  end;
  ...
  ABitmap.BeginUpdate;
  try
    // change the width of pen in order to reduce Bad Point
...
procedure DrawAngleGradient(const ABitmap: TBitmap32;
  ...
  end; { CalcVerticesForRadialLineOutline }
...
end; { HorizReflectedGradientFill }
