...
type
  TfrmSwatch = class(TForm)
    ...
    procedure imgSwatchesMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure CreateSwatchClick(Sender: TObject);
    procedure tlbtnSwatchOptionsClick(Sender: TObject);
  ...
  private
    ...
    procedure HidePopupMenuItemsForImage;
    procedure HidePopupMenuItemsForToolButton;
  public
...
procedure TfrmSwatch.HidePopupMenuItemsForImage;
begin
  dmSwatches.pmnitmResetSwatches.Visible   := False;
  dmSwatches.pmnitmReplaceSwatches.Visible := False;
  dmSwatches.pmnitmSaveChanges.Visible     := False;
  dmSwatches.pmnitmSaveSwatchesAs.Visible  := False;
  dmSwatches.pmnitmSeparator1.Visible      := False;
  dmSwatches.pmnitmDeleteSwatch.Visible    := True;
  dmSwatches.pmnitmRenameSwatch.Visible    := True;
end;

procedure TfrmSwatch.HidePopupMenuItemsForToolButton;
begin
  dmSwatches.pmnitmResetSwatches.Visible   := True;
  dmSwatches.pmnitmReplaceSwatches.Visible := True;
  dmSwatches.pmnitmSaveChanges.Visible     := True;
  dmSwatches.pmnitmSaveSwatchesAs.Visible  := True;
  dmSwatches.pmnitmSeparator1.Visible      := True;
  dmSwatches.pmnitmDeleteSwatch.Visible    := False;
  dmSwatches.pmnitmRenameSwatch.Visible    := False;
end;
  ...
  case Button of
    ...
    mbRight:
      begin
        if FSwatchList.GetSwatch(X, Y) then
        begin
          HidePopupMenuItemsForImage;

          ImageSwatchX          := X;
          ImageSwatchY          := Y;
          imgSwatches.PopupMenu := dmSwatches.pmnSwatches;
        end
        else
        begin
...
procedure TfrmSwatch.CreateSwatchClick(Sender: TObject);
var
  LColor     : TColor;
  r, g, b    : Byte;
...
begin
  ...
  if FSwatchList.Count > MAX_SWATCH_COUNT then
  begin
  ...
  begin
    ...
    LSwatchName := 'Swatch' + IntToStr(FSwatchList.NewColorSwatchCount + 1);
    LNewSwatch  := TgmSwatch.Create(r, g, b, LSwatchName);

    FSwatchList.AddSwatch(LNewSwatch);
    FSwatchList.ShowSwatches(imgSwatches);
...
procedure TfrmSwatch.tlbtnSwatchOptionsClick(Sender: TObject);
var
  p: TPoint;
begin
  HidePopupMenuItemsForToolButton;
  GetCursorPos(p);
  dmSwatches.pmnSwatches.Popup(p.X, p.Y);
end;
