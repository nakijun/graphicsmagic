...
procedure TfrmFigureProperties.DrawBrushStyle(ABitmap: TBitmap; const AStyle: TBrushStyle);
begin
...
procedure TfrmFigureProperties.SetFigureProperties(const AUnits: TgmAppliedUnit);
begin
  if Assigned(ActiveChildForm.FSelectedFigure) then
  begin
    case AUnits of
...
begin
  ...
  if Assigned(ActiveChildForm.FSelectedFigure) then
  begin
    FFigureInfo         := ActiveChildForm.LayerPanelList.FSelectedFigureInfoArray[0];
    LFigureLayerPanel   := TgmFigureLayerPanel(ActiveChildForm.LayerPanelList.Items[FFigureInfo.LayerIndex]);

    edtLayerName.Text   := LFigureLayerPanel.LayerName.Caption;
    ...
    if ActiveChildForm.FSelectedFigure.Flag in [ffRegularPolygon, ffSquare,
