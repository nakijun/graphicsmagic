...
begin
  ...
  begin
    ...
    if LLightValue in [LLowLight..LHighLight] then
    begin
    ...
    scmHLS:
      begin
        ...
        begin
          ...
          if (H in [FMinHue..FMaxHue]) and
             (L in [FMinLuminosity..FMaxLuminosity]) and
             (S in [FMinSaturation..FMaxSaturation]) then
          begin
    ...
    scmHSV:
      begin
        ...
        begin
          ...
          if (H in [FMinHue..FMaxHue]) and
             (S in [FMinSaturation..FMaxSaturation]) and
             (V in [FMinValue..FMaxValue]) then
          begin
            LIntens    := Intensity(LSrcBits^);
            LGrayValue := 255 - Round( Abs(LIntens - LMidIntens) / LFuzzIntens * 255 );
            LGrayValue := Clamp(LGrayValue, 0, 255);
    ...
    scmRGB:
      begin
        ...
        begin
          ...
          if (r in [FMinRed..FMaxRed]) and
             (g in [FMinGreen..FMaxGreen]) and
             (b in [FMinBlue..FMaxBlue]) then
          begin
            LIntens    := Intensity(LSrcBits^);
            LGrayValue := 255 - Round( Abs(LIntens - LMidIntens) / LFuzzIntens * 255 );
            LGrayValue := Clamp(LGrayValue, 0, 255);
...
begin
  ...
  begin
    ...
    begin
      if b in [FMinLuminosity..FMaxLuminosity] then
      begin
    ...
    begin
      ...
      begin
        if r in [FMinLuminosity..FMaxLuminosity] then
        begin
          LIntens := LMaskBits^ and $FF;
      ...
      begin
        if g in [FMinLuminosity..FMaxLuminosity] then
        begin
          LIntens := LMaskBits^ and $FF;
