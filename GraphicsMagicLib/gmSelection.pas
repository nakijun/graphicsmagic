...
//-- TgmRectRegionNodeList -----------------------------------------------------

  TgmRectRegionNodeList = class(TList)
  public
...
//-- TgmSelection --------------------------------------------------------------
  ...
  private
    ...
    FHorizRegionList     : TgmRectRegionNodeList;  // used for holding the rectangular regions which horizontally searched from FResizeMask
    FVertRegionList      : TgmRectRegionNodeList;  // used for holding the rectangular regions which vertically searched from FResizeMask
    FTopHorizLineList    : TgmRectRegionNodeList;  // used for holding the top lines of vertically-searched regions
    FBottomHorizLineList : TgmRectRegionNodeList;  // used for holding the bottom lines of vertically-searched regions
    FLeftVertLineList    : TgmRectRegionNodeList;  // used for holding the left lines of horizontally-searched regions
    FRightVertLineList   : TgmRectRegionNodeList;  // used for holding the right lines of horizontally-searched regions
    FMarchingAntsLineList: TgmRectRegionNodeList;  // used for holding the extended boundary lines of the selection,
                                                   // for drawing the Marching Ants selection border by the LineDDA api call
  ...
  public
    ...
    property MarchingAntsLineList: TgmRectRegionNodeList read FMarchingAntsLineList;
...
destructor TgmRectRegionNodeList.Destroy;
begin
  DeleteAll;
  inherited Destroy;
end;

procedure TgmRectRegionNodeList.DeleteAll;
var
  i    : Integer;
  LNode: TgmRectRegionNode;
begin
  if Self.Count > 0 then
  begin
    for i := (Self.Count - 1) downto 0 do
    begin
      LNode := TgmRectRegionNode(Self.Items[i]);
...
{$RANGECHECKS OFF}
  ...
  try
    ...
    begin
      ...
      begin
        LLastRegion := FHorizRegionList.Items[FHorizRegionList.Count - 1];
        LPrevRegion := FHorizRegionList.Items[FHorizRegionList.Count - 2];
...
{$RANGECHECKS OFF}
  ...
  try
    ...
    begin
      ...
      begin
        LLastRegion := FVertRegionList.Items[FVertRegionList.Count - 1];
        LPrevRegion := FVertRegionList.Items[FVertRegionList.Count - 2];
...
var
  ...
  LLineNode       : TgmRectRegionNode;
begin
  FTopHorizLineList.DeleteAll;     // clear the top-horizontal line list
  FBottomHorizLineList.DeleteAll;  // clear the bottom-horizontal line list
...
var
  ...
  LLineNode       : TgmRectRegionNode;
begin
  FLeftVertLineList.DeleteAll;   // clear the left-vertical line list
  FRightVertLineList.DeleteAll;  // clear the right-vertical line list
...
begin
  ...
  FHorizRegionList      := TgmRectRegionNodeList.Create;
  FVertRegionList       := TgmRectRegionNodeList.Create;
  FTopHorizLineList     := TgmRectRegionNodeList.Create;
  FBottomHorizLineList  := TgmRectRegionNodeList.Create;
  FLeftVertLineList     := TgmRectRegionNodeList.Create;
  FRightVertLineList    := TgmRectRegionNodeList.Create;
  FMarchingAntsLineList := TgmRectRegionNodeList.Create;
...
{$RANGECHECKS OFF}
  ...
  try
    ...
    FTopHorizLineList.DeleteAll;
    FBottomHorizLineList.DeleteAll;
    FLeftVertLineList.DeleteAll;
    FRightVertLineList.DeleteAll;
  finally
    SetLength(p, 0);
    p := nil;
  end;
...
begin
  ...
  if SquareContainsPoint( FVertices[0], ARadius, Point(AX, AY) ) then
  begin
    Result := dhAxAy;
  end
  else
  if SquareContainsPoint( FVertices[1], ARadius, Point(AX, AY) ) then
  begin
    Result := dhBxAy;
  end
  else
  if SquareContainsPoint( FVertices[2], ARadius, Point(AX, AY) ) then
  begin
    Result := dhBxBy;
  end
  else
  if SquareContainsPoint( FVertices[3], ARadius, Point(AX, AY) ) then
  begin
    Result := dhAxBy;
  end
  else
  if SquareContainsPoint( FExtraHandles[0], ARadius, Point(AX, AY) ) then
  begin
    Result := dhLeftHalfAyBy;
  end
  else
  if SquareContainsPoint( FExtraHandles[1], ARadius, Point(AX, AY) ) then
  begin
    Result := dhTopHalfAxBx;
  end
  else
  if SquareContainsPoint( FExtraHandles[2], ARadius, Point(AX, AY) ) then
  begin
    Result := dhRightHalfAyBy;
  end
  else
  if SquareContainsPoint( FExtraHandles[3], ARadius, Point(AX, AY) ) then
  begin
    Result := dhBottomHalfAxBx;
  end;
end;
