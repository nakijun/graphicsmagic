...
type
  ...
  TgmPathListState = (plsAddNewPath, plsAddNextAnchorPoint, plsAdjust);
...
//-- TgmCurveSegment -----------------------------------------------------------
  ...
  { The whole path is consists of several curve segment.
  ...
  public
    ...
    function GetHandle(const AX, AY: Integer): TgmPathSelectHandle;

    procedure DrawCurveSegment(const ACanvas: TCanvas; const AOffsetVector: TPoint);
    procedure Translate(const ATranslateVector: TPoint);
    procedure CopyCurveSegment(const ASource: TgmCurveSegment);
    procedure ScaleCurveSegment(const AOldWidth, AOldHeight, ANewWidth, ANewHeight: Integer);
...
//-- TgmCurveSegmentList -------------------------------------------------------
  ...
  { Class TgmCurveSegmentsList is for holding curve segments, use these
    curve segments to build a whole path. }
  ...
  private
    ...
    procedure DrawCurveEndingHandles(const ACanvas: TCanvas; const APenMode: TPenMode);
    procedure DrawCurveDirectionHandles(const ACanvas: TCanvas; const APenMode: TPenMode);
    procedure DeleteAllCurveSegments;
    procedure DeleteCurveSegmentByIndex(const AIndex: Integer);

    function SelectAtSingleSegment(const AX, AY: Integer): Boolean;
    // function SelectAtWholePath(const AX, AY: Integer): Boolean;
    function SelectAtHandles(const AX, AY: Integer; var ASelectedHandle: TgmPathSelectHandle): Boolean;
    function SelectAtEndPoints(const AX, AY: Integer; var ASelectedHandle: TgmPathSelectHandle): Boolean;
  public
    constructor Create;
    ...
    function AddAnchorPointOnSegment(const AX, AY: Integer): Boolean;

    function DeleteAnchorPointOnSegment(const AX, AY: Integer): Boolean;
    ...
    procedure DrawCurveDirectionLines(const ACanvas: TCanvas; const APenMode: TPenMode);
    procedure DrawCurveHandles(const ACanvas: TCanvas; const APenMode: TPenMode);

    function NilsIfPointOnBezier(const AX, AY: Integer): Boolean;

    // determine whether the mouse is point on the start point of the first curve segment
    function PointOnFirstSegmentStart(const AX, AY: Integer): Boolean;
    ...
    function PointOnLastSegmentEndingPoint(const AX, AY: Integer): TgmCurveSegment;

    // determine whether the mouse is point on a handle of direction line of a selected curve segment
    function GetSelectedDirectionHandle(const AX, AY: Integer): TgmPathSelectHandle;

    // determine whether the mouse is point on a end point of a curve segment, not a control point
    function GetSelectedEndingHandle(const AX, AY: Integer): TgmPathSelectHandle;

    // determine whether the mouse is point on any handles
    function GetSelectHandle(const AX, AY: Integer): TgmPathSelectHandle;
    ...
    procedure ConvertPoint(const AX, AY: Integer; const AOperation: TgmConvertOperation);
    procedure ClosePath;
...
//-- TgmPenPath ----------------------------------------------------------------
  ...
  { Class TgmPenPath includes a curve segment list, encapsulate these
    curve segments as a path object. }

  TgmPenPath = class(TObject)
  private
  ...
  public
    ...
    function GetSelfBackup: TgmPenPath;
...
//-- TgmPenPathList ------------------------------------------------------------
  ...
  { Class TgmPenPathList is used to hold TPenPath objects,
    each TPenPath object is a whole path. }

  TgmPenPathList = class(TList)
  private
    FSelectedPath     : TgmPenPath;
    FSelectedPathIndex: Integer;
    FPathListState    : TgmPathListState;
    FPathRegion       : HRGN;

    procedure DeleteAllPenPaths;
  public
    constructor Create;
    destructor Destroy; override;

    procedure AssignPenPathListData(const APenPathList: TgmPenPathList);
    procedure AddNewPathToList(const APath: TgmPenPath);
    procedure DeselectAllPaths;
    procedure DrawAllPathRegionsForThumbnail(const ACanvas: TCanvas; const AOffsetVector: TPoint);
    procedure DrawAllPathsForThumbnail(const ACanvas: TCanvas; const AOffsetVector: TPoint);
    procedure DrawAllPaths(const ACanvas: TCanvas; const APenMode: TPenMode; const AOffsetVector: TPoint);
    ...
    function GetCursor(const AX, AY: Integer; const APenTool: TgmPenTools): TCursor;
    ...
    function IfPointOnPathsForWholePath(const AX, AY: Integer): Integer;
    function IfPointOnPaths(const AX, AY: Integer): Boolean;
    function IfPointOnSelectedPathHandles(const AX, AY: Integer): Boolean;
    ...
    function SelectPath(const AX, AY: Integer; var ASelectedHandle: TgmPathSelectHandle): TgmPenPath;

    function LoadPenPathsFromStream(const AMemoryStream: TMemoryStream;
      const AVersionNum: Integer): Boolean;

    procedure SavePenPathsToStream(const AMemoryStream: TMemoryStream);

    function UpdatePathRegion(const ACanvasWidth, ACanvasHeight: Integer;
      const AOffsetVector: TPoint): Boolean;

    property SelectedPath     : TgmPenPath       read FSelectedPath      write FSelectedPath;
    property SelectedPathIndex: Integer          read FSelectedPathIndex write FSelectedPathIndex;
    property PathListState    : TgmPathListState read FPathListState     write FPathListState;
    property PathRegion       : HRGN             read FPathRegion;
  end;

//-- Common Methods ------------------------------------------------------------

  function GetPenPathCursor(const ASelectHandle: TgmPathSelectHandle): TCursor;
...
var
  ...
  LTempPenStyle                     : TPenStyle;
  LTempPenMode                      : TPenMode;
  LStartPoint, LControl1, LOpposite1: TPoint;
  LEndPoint, LControl2, LOpposite2  : TPoint;
begin
  if Self.Count > 0 then
  begin
    with ACanvas do
...
constructor TgmPenPath.Create;
begin
  inherited Create;
  FCurveSegmentsList := TgmCurveSegmentsList.Create;
end;

destructor TgmPenPath.Destroy;
begin
  FCurveSegmentsList.Free;
  inherited Destroy;
end;

function TgmPenPath.GetSelfBackup: TgmPenPath;
begin
  Result := TgmPenPath.Create;
...
{ Class TgmPenPathList is used to hold TPenPath objects,
  each TgmPenPath object is a whole path. }

constructor TgmPenPathList.Create;
begin
  inherited Create;
  
  FSelectedPath      := nil;
  FSelectedPathIndex := -1;
  FPathListState     := plsAddNewPath;
  FPathRegion        := 0;
end;

destructor TgmPenPathList.Destroy;
begin
  DeleteAllPenPaths;
...
procedure TgmPenPathList.DeleteAllPenPaths;
var
  i       : Integer;
  LPenPath: TgmPenPath;
begin
  ...
  begin
    ...
    begin
      LPenPath := TgmPenPath(Self.Items[i]);
      LPenPath.Free;
    end;
...
procedure TgmPenPathList.AssignPenPathListData(
  const APenPathList: TgmPenPathList);
var
  i             : Integer;
  LPenPath      : TgmPenPath;
  LPenPathBackup: TgmPenPath;
begin
  if Assigned(APenPathList) then
  begin
    DeleteAllPenPaths;
    
    if APenPathList.Count > 0 then
    begin
      for i := 0 to (APenPathList.Count - 1) do
      begin
        LPenPath       := TgmPenPath(APenPathList.Items[i]);
        LPenPathBackup := LPenPath.GetSelfBackup;

        Self.Add(LPenPathBackup);
      end;
    end;

    FPathListState     := APenPathList.PathListState;
    FSelectedPathIndex := APenPathList.SelectedPathIndex;

    if FSelectedPathIndex >= 0 then
    begin
      FSelectedPath := TgmPenPath(Self.Items[FSelectedPathIndex]);
    end
    else
    begin
      FSelectedPath := nil;
...
procedure TgmPenPathList.AddNewPathToList(const APath: TgmPenPath);
begin
  Self.Add(APath);
  
  FSelectedPath      := APath;
  FSelectedPathIndex := Self.Count - 1;
  FPathListState     := plsAddNextAnchorPoint;
end;

procedure TgmPenPathList.DeselectAllPaths;
var
  i       : Integer;
  LPenPath: TgmPenPath;
begin
  ...
  begin
    ...
    begin
      LPenPath                                 := TgmPenPath(Self.Items[i]);
      LPenPath.CurveSegmentsList.IsSelected    := False;
      LPenPath.CurveSegmentsList.IsSelectedAll := False;
      FSelectedPath                            := nil;
      FSelectedPathIndex                       := -1;
      FPathListState                           := plsAddNewPath;
    end;
...
procedure TgmPenPathList.DrawAllPathRegionsForThumbnail(
...
begin
  ...
  begin
    ...
    begin
      ...
      LPenPath.CurveSegmentsList.MakeRegion(ACanvas, AOffsetVector);
    end;
  end;
end;

procedure TgmPenPathList.DrawAllPathsForThumbnail(
  const ACanvas: TCanvas; const AOffsetVector: TPoint);
var
  i       : Integer;
  LPenPath: TgmPenPath;
begin
  ...
  begin
    ...
    begin
      ...
      LPenPath.CurveSegmentsList.DrawCurveSegments(ACanvas, pmCopy, AOffsetVector);
    end;
  end;
end; 

procedure TgmPenPathList.DrawAllPaths(const ACanvas: TCanvas;
  const APenMode: TPenMode; const AOffsetVector: TPoint);
var
  i       : Integer;
  LPenPath: TgmPenPath;
begin
  ...
  begin
    ...
    begin
      ...
      with LPenPath.CurveSegmentsList do
      begin
...
procedure TgmPenPathList.TranslateAllSelectedPaths(
  const ATranslateVector: TPoint);
var
  i       : Integer;
  LPenPath: TgmPenPath;
begin
  ...
  begin
    ...
    begin
      ...
      begin
        LPenPath.CurveSegmentsList.TranslateAllSegments(ATranslateVector);
      end;
    end;
  end;
end;

procedure TgmPenPathList.TranslateAllPaths(const ATranslateVector: TPoint);
var
  i       : Integer;
  LPenPath: TgmPenPath;
begin
...
procedure TgmPenPathList.ScaleAllPaths(
  const AOldWidth, AOldHeight, ANewWidth, ANewHeight: Integer);
var
  i       : Integer;
  LPenPath: TgmPenPath;
begin
  ...
  begin
    ...
    begin
      ...
      LPenPath.CurveSegmentsList.ScaleAllSegments(AOldWidth, AOldHeight,
                                                  ANewWidth, ANewHeight);
    end;
  end;
end;

procedure TgmPenPathList.SelectWholePathByIndex(const AIndex: Integer);
var
  LPenPath: TgmPenPath;
begin
  if Self.Count > 0 then
  begin
    if (AIndex >= 0) and (AIndex < Self.Count) then
    begin
      LPenPath := TgmPenPath(Self.Items[AIndex]);
      
      LPenPath.CurveSegmentsList.IsSelectedAll := True;
      LPenPath.CurveSegmentsList.IsSelected    := False;
    end;
  end;
end;

procedure TgmPenPathList.SelectPathByIndex(const AIndex: Integer);
begin
  if Self.Count > 0 then
  begin
    if (AIndex >= 0) and (AIndex < Self.Count) then
    begin
      ...
      FSelectedPath := TgmPenPath(Self.Items[AIndex]);
...
function TgmPenPathList.GetWholeSelectedPathsCount: Integer;
var
  i       : Integer;
  LPenPath: TgmPenPath;
begin
...
function TgmPenPathList.GetSelectedPathsCount: Integer;
var
  i       : Integer;
  LPenPath: TgmPenPath;
begin
  ...
  begin
    ...
    begin
      ...
      if LPenPath.CurveSegmentsList.IsSelectedAll or
         LPenPath.CurveSegmentsList.IsSelected then
      begin
        Inc(Result);
      end;
    end;
...
procedure TgmPenPathList.StrokeSelectedPaths(const ACanvas: TCanvas;
  const APenWidth: Integer; const APenColor, ABrushColor: TColor;
  const APenStyle: TPenStyle; const ABrushStyle: TBrushStyle;
  const AOffsetVector: TPoint);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      LPenPath.CurveSegmentsList.StrokePathWithForegroundColor(ACanvas,
        APenWidth, APenColor, ABrushColor, APenStyle, ABrushStyle, AOffsetVector);
    end;
  end;
end; 

procedure TgmPenPathList.FillSelectedPaths(const ACanvas: TCanvas;
  const AColor: TColor; const ABrushStyle: TBrushStyle;
  const AOffsetVector: TPoint);
var
  i       : Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      LPenPath.CurveSegmentsList.FillPathWithForegroundColor(
...
// get a cursor according to a selected path and path tools
function TgmPenPathList.GetCursor(const AX, AY: Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      if LPenPath.CurveSegmentsList.NilsIfPointOnBezier(AX, AY) then
      begin
        case APenTool of
          ptPathComponentSelection:
            begin
...
{ Determine whether the mouse is point on a path, used to select a whole
  path. The return value is the index of selected path in the list. }
function TgmPenPathList.IfPointOnPathsForWholePath(
...
function TgmPenPathList.IfPointOnPaths(const AX, AY: Integer): Boolean;
var
  i       : Integer;
  LPenPath: TgmPenPath;
begin
...
function TgmPenPathList.IfPointOnSelectedPathHandles(
  const AX, AY: Integer): Boolean;
var
  i       : Integer;
  LPenPath: TgmPenPath;
begin
  ...
  begin
    ...
    begin
      ...
      if LPenPath.CurveSegmentsList.GetSelectHandle(AX, AY) <> pshNone then
      begin
        Result := True;
        Break;
      end;
...
function TgmPenPathList.IfSelectedPathByIndex(const AIndex: Integer): Boolean;
var
  LPenPath: TgmPenPath;
begin
  Result := False;
  ...
  begin
    ...
    begin
      LPenPath := TgmPenPath(Self.Items[AIndex]);
      Result   := LPenPath.CurveSegmentsList.IsSelected;
    end;
  end;
end;

function TgmPenPathList.IfSelectedWholePathByIndex(
...
begin
  ...
  begin
    ...
    begin
      LPenPath := TgmPenPath(Self.Items[AIndex]);
      Result   := LPenPath.CurveSegmentsList.IsSelectedAll;
    end;
  end;
end;

function TgmPenPathList.SelectPath(const AX, AY: Integer;
  var ASelectedHandle: TgmPathSelectHandle): TgmPenPath;
var
  i       : Integer;
  LPenPath: TgmPenPath;
begin
  ...
  begin
    ...
    begin
      ...
      if LPenPath.CurveSegmentsList.SelectAtSingleSegment(AX, AY) then
      begin
        // if point on the start point of the first curve segment, then select it only
        if LPenPath.CurveSegmentsList.PointOnFirstSegmentStart(AX, AY) then
        begin
          LPenPath.CurveSegmentsList.SelectAtHandles(AX, AY, ASelectedHandle);
        end;

        FSelectedPath      := LPenPath;
...
function TgmPenPathList.LoadPenPathsFromStream(
  const AMemoryStream: TMemoryStream; const AVersionNum: Integer): Boolean;
var
  i, LLoadCount: Integer;
  LPenPath     : TgmPenPath;
begin
  ...
  if LLoadCount > 0 then
  begin
    Self.DeleteAllPenPaths;
    
    for i := 0 to (LLoadCount - 1) do
    begin
      LPenPath := TgmPenPath.Create;

      LPenPath.CurveSegmentsList.LoadCurveSegmentsFromStream(AMemoryStream, AVersionNum);
      Self.Add(LPenPath);
    end;
  end;
...
procedure TgmPenPathList.SavePenPathsToStream(
  const AMemoryStream: TMemoryStream);
var
  i       : Integer;
  LPenPath: TgmPenPath;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        LPenPath.CurveSegmentsList.SaveCurveSegmentsToStream(AMemoryStream);
      end;
    end;
  end;
end;
...
function TgmPenPathList.UpdatePathRegion(
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if LPenPath.CurveSegmentsList.IsSelected or
           LPenPath.CurveSegmentsList.IsSelectedAll then
        begin
          // remove the old data, prepare to store new data
          SetLength(LCurvePoints, 0);

          if LPenPath.CurveSegmentsList.Count > 0 then
          begin
            for j := 0 to (LPenPath.CurveSegmentsList.Count - 1) do
            begin
              // get each curve of the path
              LSegment := TgmCurveSegment(LPenPath.CurveSegmentsList.Items[j]);
