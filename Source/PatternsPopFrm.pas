...
type
  ...
  public
    procedure ShowSelectedPattern(const APatternList: TgmPatternList;
      const AImage: TImage32; const AImageHolder: TPanel;
      const ZoomSize: Integer);
...
procedure TfrmPatterns.ShowSelectedPattern(const APatternList: TgmPatternList;
  const AImage: TImage32; const AImageHolder: TPanel; const ZoomSize: Integer);
var
...
begin
  ...
  try
    ...
    begin
      GetScaledBitmap(APatternList.SelectedPattern.Bitmap, LTempBitmap,
                      ZoomSize, ZoomSize);
                      
      AImage.Bitmap.Assign(LTempBitmap);
...
begin
  ...
  try
    LUseInternalPattern := StrToInt( ReadInfoFromIniFile(SECTION_STAMP_PATTERN, IDENT_STAMP_PATTERN_USE_INTERNAL, '1') );
  except
    LUseInternalPattern := 1;
    WriteInfoToIniFile( SECTION_STAMP_PATTERN, IDENT_STAMP_PATTERN_USE_INTERNAL, IntToStr(LUseInternalPattern) );
  end;
  ...
  begin
    LFileName := ReadInfoFromIniFile(SECTION_STAMP_PATTERN, IDENT_STAMP_OPEN_PATTERN_FILE, '');
  ...
  // load selected index
  try
    LIndex := StrToInt( ReadInfoFromIniFile(SECTION_STAMP_PATTERN, IDENT_STAMP_PATTERN_INDEX, '0') );
  except
    LIndex := 0;
    WriteInfoToIniFile( SECTION_STAMP_PATTERN, IDENT_STAMP_PATTERN_INDEX, IntToStr(LIndex) );
  end;

  // load thumbnail info
  try
    LThumbnailSizeMode := TgmThumbnailSizeMode(  StrToInt( ReadInfoFromIniFile(SECTION_STAMP_PATTERN, IDENT_STAMP_PATTERN_THUMBNAIL_SIZE_MODE, '0') )  );

    if not (LThumbnailSizeMode in [tsmSmall, tsmLarge]) then
    begin
      LThumbnailSizeMode := tsmLarge;
      WriteInfoToIniFile(  SECTION_STAMP_PATTERN, IDENT_STAMP_PATTERN_THUMBNAIL_SIZE_MODE, IntToStr( Integer(LThumbnailSizeMode) )  );
    end;
  except
    LThumbnailSizeMode := tsmLarge;
    WriteInfoToIniFile(  SECTION_STAMP_PATTERN, IDENT_STAMP_PATTERN_THUMBNAIL_SIZE_MODE, IntToStr( Integer(LThumbnailSizeMode) )  );
  ...
  begin
    ...
    ShowSelectedPattern(FPatternList, frmMain.imgPatternForStamp,
...
begin
  ...
  try
    LUseInternalPattern := StrToInt( ReadInfoFromIniFile(SECTION_FILL_DIALOG, IDENT_FILL_PATTERN_USE_INTERNAL, '1') );
  except
    LUseInternalPattern := 1;
    WriteInfoToIniFile( SECTION_FILL_DIALOG, IDENT_FILL_PATTERN_USE_INTERNAL, IntToStr(LUseInternalPattern) );
  end;
  ...
  begin
    LFileName := ReadInfoFromIniFile(SECTION_FILL_DIALOG, IDENT_FILL_OPEN_PATTERN_FILE, '');
    FPatternList.LoadFromFile(LFileName);
  ...
  // load selected index
  try
    LIndex := StrToInt( ReadInfoFromIniFile(SECTION_FILL_DIALOG, IDENT_FILL_PATTERN_INDEX, '0') );
  except
    LIndex := 0;
    WriteInfoToIniFile( SECTION_FILL_DIALOG, IDENT_FILL_PATTERN_INDEX, IntToStr(LIndex) );
  end;

  // load thumbnail info
  try
    LThumbnailSizeMode := TgmThumbnailSizeMode(  StrToInt( ReadInfoFromIniFile(SECTION_FILL_DIALOG, IDENT_FILL_PATTERN_THUMBNAIL_SIZE_MODE, '0') )  );

    if not (LThumbnailSizeMode in [tsmSmall, tsmLarge]) then
    begin
      LThumbnailSizeMode := tsmLarge;
      WriteInfoToIniFile(  SECTION_FILL_DIALOG, IDENT_FILL_PATTERN_THUMBNAIL_SIZE_MODE, IntToStr( Integer(LThumbnailSizeMode) )  );
    end;
  except
    LThumbnailSizeMode := tsmLarge;
    WriteInfoToIniFile(  SECTION_FILL_DIALOG, IDENT_FILL_PATTERN_THUMBNAIL_SIZE_MODE, IntToStr( Integer(LThumbnailSizeMode) )  );
...
begin
  ...
  try
    LUseInternalPattern := StrToInt( ReadInfoFromIniFile(SECTION_PAINT_BUCKET, IDENT_PAINT_BUCKET_PATTERN_USE_INTERNAL, '1') );
  except
    LUseInternalPattern := 1;
    WriteInfoToIniFile( SECTION_PAINT_BUCKET, IDENT_PAINT_BUCKET_PATTERN_USE_INTERNAL, IntToStr(LUseInternalPattern) );
  end;
  ...
  begin
    LFileName := ReadInfoFromIniFile(SECTION_PAINT_BUCKET, IDENT_PAINT_BUCKET_OPEN_PATTERN_FILE, '');
    FPatternList.LoadFromFile(LFileName);

    // If failure in loading external Patterns, then load the internal Patterns.
    if FPatternList.IsUsingInternal then
    begin
      WriteInfoToIniFile(SECTION_PAINT_BUCKET, IDENT_PAINT_BUCKET_PATTERN_USE_INTERNAL, '1');
    end;
  end;

  // load selected index
  try
    LIndex := StrToInt( ReadInfoFromIniFile(SECTION_PAINT_BUCKET, IDENT_PAINT_BUCKET_PATTERN_INDEX, '0') );
  except
    LIndex := 0;
    WriteInfoToIniFile( SECTION_PAINT_BUCKET, IDENT_PAINT_BUCKET_PATTERN_INDEX, IntToStr(LIndex) );
  end;

  // load thumbnail info
  try
    LThumbnailSizeMode := TgmThumbnailSizeMode(  StrToInt( ReadInfoFromIniFile(SECTION_PAINT_BUCKET, IDEMT_PAINT_BUCKET_PATTERN_THUMBNAIL_SIZE_MODE, '0') )  );

    if not (LThumbnailSizeMode in [tsmSmall, tsmLarge]) then
    begin
      LThumbnailSizeMode := tsmLarge;
      WriteInfoToIniFile(  SECTION_PAINT_BUCKET, IDEMT_PAINT_BUCKET_PATTERN_THUMBNAIL_SIZE_MODE, IntToStr( Integer(LThumbnailSizeMode) )  );
    end;
  except
    LThumbnailSizeMode := tsmLarge;
    WriteInfoToIniFile(  SECTION_PAINT_BUCKET, IDEMT_PAINT_BUCKET_PATTERN_THUMBNAIL_SIZE_MODE, IntToStr( Integer(LThumbnailSizeMode) )  );
  ...
  begin
    ...
    ShowSelectedPattern(FPatternList, frmMain.imgPatternForPaintBucket,
...
begin
  ...
  try
    LUseInternalPattern := StrToInt( ReadInfoFromIniFile(SECTION_PATTERN_LAYER, IDENT_LAYER_PATTERN_USE_INTERNAL, '1') );
  except
    LUseInternalPattern := 1;
    WriteInfoToIniFile( SECTION_PATTERN_LAYER, IDENT_LAYER_PATTERN_USE_INTERNAL, IntToStr(LUseInternalPattern) );
  end;
  ...
  begin
    LFileName := ReadInfoFromIniFile(SECTION_PATTERN_LAYER, IDENT_LAYER_OPEN_PATTERN_FILE, '');
    FPatternList.LoadFromFile(LFileName);

    // If failure in loading external Patterns, then load the internal Patterns.
    if FPatternList.IsUsingInternal then
    begin
      WriteInfoToIniFile(SECTION_PATTERN_LAYER, IDENT_LAYER_PATTERN_USE_INTERNAL, '1');
    end;
  end;

  // load selected index
  try
    LIndex := StrToInt( ReadInfoFromIniFile(SECTION_PATTERN_LAYER, IDENT_LAYER_PATTERN_INDEX, '0') );
  except
    LIndex := 0;
    WriteInfoToIniFile( SECTION_PATTERN_LAYER, IDENT_LAYER_PATTERN_INDEX, IntToStr(LIndex) );
  end;

  // load thumbnail info
  try
    LThumbnailSizeMode := TgmThumbnailSizeMode(  StrToInt( ReadInfoFromIniFile(SECTION_PATTERN_LAYER, IDENT_LAYER_PATTERN_THUMBNAIL_SIZE_MODE, '0') )  );

    if not (LThumbnailSizeMode in [tsmSmall, tsmLarge]) then
    begin
      LThumbnailSizeMode := tsmLarge;
      WriteInfoToIniFile(  SECTION_PATTERN_LAYER, IDENT_LAYER_PATTERN_THUMBNAIL_SIZE_MODE, IntToStr( Integer(LThumbnailSizeMode) )  );
    end;
  except
    LThumbnailSizeMode := tsmLarge;
    WriteInfoToIniFile(  SECTION_PATTERN_LAYER, IDENT_LAYER_PATTERN_THUMBNAIL_SIZE_MODE, IntToStr( Integer(LThumbnailSizeMode) )  );
  ...
  begin
    ...
    ShowSelectedPattern(FPatternList, frmPatternFill.imgPattern,
    ...
    begin
      ...
      begin
        ...
        begin
          TgmPatternLayerPanel(LayerPanelList.SelectedLayerPanel).PatternBitmap := FPatternList.SelectedPattern.Bitmap;
        end;
      end;
    end;
  end;
      ...
      case FPatternListUser of
        pluFill:
          begin
            ...
            begin
              WriteInfoToIniFile(SECTION_FILL_DIALOG, IDENT_FILL_PATTERN_USE_INTERNAL, '0');
            end;
          end;
        ...
        pluStamp:
          begin
            ...
            WriteInfoToIniFile( SECTION_STAMP_PATTERN, IDENT_STAMP_PATTERN_INDEX,
                                IntToStr(FPatternList.SelectedIndex) );
            ...
            begin
              WriteInfoToIniFile(SECTION_STAMP_PATTERN, IDENT_STAMP_PATTERN_USE_INTERNAL, '0');
            end;
          end;
        ...
        pluPaintBucket:
          begin
            ...
            begin
              WriteInfoToIniFile(SECTION_PAINT_BUCKET, IDENT_PAINT_BUCKET_PATTERN_USE_INTERNAL, '0');
            end;
          end;
        ...
        pluPatternLayer:
          begin
            ...
            WriteInfoToIniFile( SECTION_PATTERN_LAYER, IDENT_LAYER_PATTERN_INDEX,
                                IntToStr(FPatternList.SelectedIndex) );
            ...
            begin
              WriteInfoToIniFile(SECTION_PATTERN_LAYER, IDENT_LAYER_PATTERN_USE_INTERNAL, '0');
            end;

            if ActiveChildForm.LayerPanelList.SelectedLayerPanel.LayerFeature = lfPattern then
            begin
              TgmPatternLayerPanel(ActiveChildForm.LayerPanelList.SelectedLayerPanel).PatternBitmap := FLayerPattern;
            end;
          end;
        end;
    end;
  ...
  begin
    ...
    begin
      ...
      case FPatternListUser of
        pluFill:
          begin
            WriteInfoToIniFile(SECTION_FILL_DIALOG, IDENT_FILL_OPEN_PATTERN_FILE, AFileName);
            WriteInfoToIniFile(SECTION_FILL_DIALOG, IDENT_FILL_PATTERN_USE_INTERNAL, '0');
          end;

        pluStamp:
          begin
            WriteInfoToIniFile(SECTION_STAMP_PATTERN, IDENT_STAMP_OPEN_PATTERN_FILE, AFileName);
            WriteInfoToIniFile(SECTION_STAMP_PATTERN, IDENT_STAMP_PATTERN_USE_INTERNAL, '0');
          end;

        pluPaintBucket:
          begin
            WriteInfoToIniFile(SECTION_PAINT_BUCKET, IDENT_PAINT_BUCKET_OPEN_PATTERN_FILE, AFileName);
            WriteInfoToIniFile(SECTION_PAINT_BUCKET, IDENT_PAINT_BUCKET_PATTERN_USE_INTERNAL, '0');
          end;

        pluPatternLayer:
          begin
            WriteInfoToIniFile(SECTION_PATTERN_LAYER, IDENT_LAYER_OPEN_PATTERN_FILE, AFileName);
            WriteInfoToIniFile(SECTION_PATTERN_LAYER, IDENT_LAYER_PATTERN_USE_INTERNAL, '0');
          end;
      end;
    end;
      ...
      begin
        ...
        begin
          MessageDlg('Cannot load the bitmap, bacause the width or' + #10#13 +
                     'height of the bitmap is larger than 200 pixels.', mtError, [mbOK], 0);
        end
        else
        begin
...
var
  LScaleBitmap: TBitmap32;
begin
  LScaleBitmap   := TBitmap32.Create;
  ...
  try
    ...
    begin
      ...
      GetScaledBitmap(FPatternList.SelectedPattern.Bitmap, LScaleBitmap,
                      frmPatternName.pnlPatternView.Width - 4,
                      frmPatternName.pnlPatternView.Height - 4);
    end;

    frmPatternName.imgPatternView.Bitmap.Assign(LScaleBitmap);
...
begin
  ...
  case FPatternListUser of
    pluFill:
      begin
        WriteInfoToIniFile(  SECTION_FILL_DIALOG, IDENT_FILL_PATTERN_THUMBNAIL_SIZE_MODE,
                             IntToStr( Integer(LThumbnailSizeMode) )  );
      end;

    pluStamp:
      begin
        WriteInfoToIniFile(  SECTION_STAMP_PATTERN, IDENT_STAMP_PATTERN_THUMBNAIL_SIZE_MODE,
                             IntToStr( Integer(LThumbnailSizeMode) )  );
      end;

    pluPaintBucket:
      begin
        WriteInfoToIniFile(  SECTION_PAINT_BUCKET, IDEMT_PAINT_BUCKET_PATTERN_THUMBNAIL_SIZE_MODE,
                             IntToStr( Integer(LThumbnailSizeMode) )  );
      end;

    pluPatternLayer:
      begin
        WriteInfoToIniFile(  SECTION_PATTERN_LAYER, IDENT_LAYER_PATTERN_THUMBNAIL_SIZE_MODE,
                             IntToStr( Integer(LThumbnailSizeMode) )  );
      end;
  end;
end; 
...
begin
  ...
  begin
    ...
    if (LIndex >= 0) and (LIndex <> FPatternList.SelectedIndex) and
       (LBmpCoord.X >= 0) and (LBmpCoord.X < imgvwPatterns.Bitmap.Width) and
       (LBmpCoord.Y >= 0) and (LBmpCoord.Y < imgvwPatterns.Bitmap.Height) then
    begin
      FPatternList.SelectPatternByIndex(LIndex);
      ...
      case FPatternListUser of
        pluFill:
          begin
            ...
            ShowSelectedPattern(FPatternList, frmFill.imgSelectedPattern,
                                frmFill.pnlCustomPattern, frmFill.pnlCustomPattern.Width - 4);
    ...
    begin
      if (LIndex >= 0) and (LIndex < FPatternList.Count) and
         (LBmpCoord.X >= 0) and (LBmpCoord.X < imgvwPatterns.Bitmap.Width) and
         (LBmpCoord.Y >= 0) and (LBmpCoord.Y < imgvwPatterns.Bitmap.Height) then
      begin
